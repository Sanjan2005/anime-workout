    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative z-0">
      ...
      >
        <Card className="w-full max-w-md bg-card/80 backdrop-blur-md border-violet-500/20 shadow-[0_0_40px_rgba(139,92,246,0.1)]">
          <CardHeader>
            <CardTitle className="font-orbitron tracking-wide text-2xl">Log In</CardTitle>
            <CardDescription className="text-muted-foreground">Continue your training arc</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="pt-2">
                <Button 
                  ...
                  disabled={loading}
                >
                  <span className="relative z-10">{loading ? "Powering up..." : "Sign In"}</span>
            ...
            <p className="text-sm text-muted-foreground text-center mt-6">
              No account?{" "}
              <Link href="/signup" className="text-violet-400 hover:text-violet-300 hover:underline hover:drop-shadow-[0_0_8px_rgba(167,139,250,0.8)] transition-all">
                Sign up
              </Link>
            </p>
          </CardContent>
